...
class Player {
	...
	public void play() {
		if(isBot) {
			...
			Main.playedApple.add(new PlayedApple(playerID, hand.get(0)));
			hand.remove(0);
		} else if(online){
			try {
				String aPlayedApple = inFromClient.readLine();
				Main.playedApple.add(new PlayedApple(playerID, aPlayedApple));					
			} catch (Exception e) {}
			...
			Main.playedApple.add(new PlayedApple(playerID, hand.get(choice)));
			hand.remove(choice);
	...
	public PlayedApple judge() {
		if(isBot){
			...
			return Main.playedApple.get(choice);			
		}
	}
...
public class Main {
	 ...
	 **/
	public Main(int numberOfOnlinePlayers) throws Exception {
  		redApples = new ArrayList<String>(Files.readAllLines(Paths.get("src/main/resources/greenApples.txt"), StandardCharsets.ISO_8859_1)); 
  		greenApples = new ArrayList<String>(Files.readAllLines(Paths.get("src/main/resources/greenApples.txt"), StandardCharsets.ISO_8859_1));
